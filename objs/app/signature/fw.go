...
func (c *FwSignature) Get(vsys, app, name string) (Entry, error) {
	...
	return c.details(c.con.Get, vsys, app, name)
}

// Show performs SHOW to retrieve information for the given uid.
func (c *FwSignature) Show(vsys, app, name string) (Entry, error) {
	c.con.LogQuery("(show) %s %q", singular, name)
	return c.details(c.con.Show, vsys, app, name)
}
...
func (c *FwSignature) Edit(vsys, app string, e Entry) error {
	...
	// Set xpath.
	path := c.xpath(vsys, app, []string{e.Name})
...
func (c *FwSignature) versioning() (normalizer, func(Entry) interface{}) {
	return &container_v1{}, specify_v1
}

func (c *FwSignature) details(fn util.Retriever, vsys, app, name string) (Entry, error) {
	path := c.xpath(vsys, app, []string{name})
	obj, _ := c.versioning()
...
func (c *FwSignature) xpath(vsys, app string, vals []string) []string {
	ans := make([]string, 0, 9)
