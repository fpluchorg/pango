...
// Panorama is the client.Objects.SecurityProfileGroup namespace.
...
func (c *Panorama) FromPanosConfig(dg, name string) (Entry, error) {
	ans := c.container()
	err := c.ns.FromPanosConfig(c.pather(dg), name, ans)
	return first(ans, err)
}

// AllFromPanosConfig retrieves all objects stored in the retrieved config.
func (c *Panorama) AllFromPanosConfig(dg string) ([]Entry, error) {
	ans := c.container()
	err := c.ns.AllFromPanosConfig(c.pather(dg), ans)
	return all(ans, err)
}
