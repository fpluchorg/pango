...
func (c *Panorama) FromPanosConfig(tmpl, ts, iType, eth, name string) (Entry, error) {
	ans := c.container()
	err := c.ns.FromPanosConfig(c.pather(tmpl, ts, iType, eth), name, ans)
	return first(ans, err)
}

// AllFromPanosConfig retrieves all objects stored in the retrieved config.
func (c *Panorama) AllFromPanosConfig(tmpl, ts, iType, eth string) ([]Entry, error) {
	ans := c.container()
	err := c.ns.AllFromPanosConfig(c.pather(tmpl, ts, iType, eth), ans)
	return all(ans, err)
}
