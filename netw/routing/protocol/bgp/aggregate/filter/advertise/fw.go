...
// Firewall is the client.Network.BgpAggAdvertiseFilter namespace.
...
func (c *Firewall) FromPanosConfig(vr, ag, name string) (Entry, error) {
	ans := c.container()
	err := c.ns.FromPanosConfig(c.pather(vr, ag), name, ans)
	return first(ans, err)
}
