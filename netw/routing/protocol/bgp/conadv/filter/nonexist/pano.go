...
// Panorama is the client.Network.BgpConAdvNonExistFilter namespace.
...
func (c *Panorama) FromPanosConfig(tmpl, ts, vr, ca, name string) (Entry, error) {
	ans := c.container()
	err := c.ns.FromPanosConfig(c.pather(tmpl, ts, vr, ca), name, ans)
	return first(ans, err)
}

// AllFromPanosConfig retrieves all objects stored in the retrieved config.
func (c *Panorama) AllFromPanosConfig(tmpl, ts, vr, ca string) ([]Entry, error) {
	ans := c.container()
	err := c.ns.AllFromPanosConfig(c.pather(tmpl, ts, vr, ca), ans)
	return all(ans, err)
}
